...
app.get("/api/bills", async (req, reply) => {
  ...
  if (query) {
    params.push(`%${query}%`);
    where.push(
      `(title ILIKE $${params.length} OR url ILIKE $${params.length})`
    );
  }
  if (date_from) {
...
app.get("/api/bills/:id/versions", async (req, reply) => {
  ...
  const res = await q(
    `SELECT id, fetched_at, diff_from_prev, left(content, 4000) AS content_preview
     FROM bill_versions WHERE bill_id = $1 ORDER BY fetched_at DESC`,
    [id]
  );
  //reply.send(res.rows);
  return res.rows;
