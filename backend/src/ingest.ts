...
// import { BillLite } from "./types.js";

// export async function ingestPeriod(daysBack = 2): Promise<BillLite[]> {
...
//   const items: BillLite[] = [];

//   // Кожен рядок таблиці: [0]=euro-box, [1]=номер (a.link-blue), [2]=дата, [3]=назва, [4]=суб'єкт
...
//     const title = titleRaw.replace(/\s+/g, " ").trim(); // прибрати \n та зайві пробіли
...
//     items.push({ radaId, url, title, number: regNum });
//   });

//   // upsert bill skeleton (записуємо ще number)
//   for (const it of items) {
//     await q(
//       `INSERT INTO bills (rada_id, title, url, number)
//        VALUES ($1,$2,$3,$4)
//        ON CONFLICT (rada_id)
//        DO UPDATE SET
...
//       [it.radaId, it.title, it.url, it.number ?? null]
//     );
//   }
//   return items;
// }

// export async function ingestBillDetails(
//   url: string,
//   radaId: string
// ): Promise<number> {
//   const browser = await chromium.launch();
...
//   const title =
//     $("h1, .bill-title").first().text().trim() || $("title").text().trim();
...
//     `UPDATE bills SET title = COALESCE($1, title), status = $2
//      WHERE rada_id = $3
//      RETURNING id`,
//     [title || null, status, radaId]
//   );
//   const billId = upd.rows[0].id;

//   // insert version if new
//   const hash = sha256(content);
//   const prev = await q<{ content_hash: string }>(
//     `SELECT content_hash FROM bill_versions WHERE bill_id = $1 ORDER BY fetched_at DESC LIMIT 1`,
//     [billId]
//   );
//   if (!prev.rows[0] || prev.rows[0].content_hash !== hash) {
//     await q(
//       `INSERT INTO bill_versions (bill_id, content_hash, content)
//        VALUES ($1,$2,$3)
...
// function parseUaDate(d: string): string | null {
...
//     const isoDate = parseUaDate(dateText); // 'YYYY-MM-DD' або null

//     // Назва ЗП (очищена від переносів і зайвих пробілів)
//     const titleRaw = $(tds[3]).text();
...
//   // upsert bill skeleton (записуємо title, url, number, registered_at)
//   for (const it of items) {
//     await q(
//       `INSERT INTO bills (rada_id, title, url, number, registered_at)
//        VALUES ($1,$2,$3,$4,$5)
//        ON CONFLICT (rada_id)
//        DO UPDATE SET
//          title         = EXCLUDED.title,
...
//       [it.radaId, it.title, it.url, it.number ?? null, it.registeredAt ?? null]
//     );
//   }
...
//   // оновлюємо bill (тут не чіпаємо registered_at — воно з таблиці періоду)
//   const upd = await q<{ id: number }>(
//     `UPDATE bills SET title = COALESCE($1, title), status = $2
//      WHERE rada_id = $3
//      RETURNING id`,
...
async function getFirstHtmlWithPlaywright(url: string): Promise<string> {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage();
    await page.goto(url, { waitUntil: "domcontentloaded" });
...
function collectFromHtml(html: string): {
  ...
  const startVal =
    $("#PeriodSite_start").val()?.toString() ||
    $("#startDate").val()?.toString() ||
    undefined;
  const endVal =
    $("#PeriodSite_end").val()?.toString() ||
...
export async function ingestPeriod(daysBack = 2): Promise<BillLite[]> {
  ...
  const all: Record<string, BillLite> = {};
  for (const it of first.items) all[it.radaId] = it;
  ...
  if (!startVal || !endVal) {
    console.warn(
      "Не знайдено PeriodSite_start/end — лишаємо тільки першу сторінку."
    );
  } else {
    for (let p = 2; p <= first.maxPage; p++) {
      try {
        ...
        for (const it of items) all[it.radaId] = it; // дедуп по radaId
      } catch (e: any) {
  ...
  for (const it of items) {
    await q(
      `INSERT INTO bills (rada_id, title, url, number, registered_at, law_title)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (rada_id)
       DO UPDATE SET
         title         = EXCLUDED.title,
